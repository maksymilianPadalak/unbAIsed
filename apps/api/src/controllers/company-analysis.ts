...
export const analyzeBatchCompanies = async (
  ...
  try {
    ...
    console.log(
      `🚀 Starting batch analysis for ${companies.length} companies:`,
      companies
    );
    ...
    const analysisPromises = companies.map(async (company: string) => {
      try {
        ...
        const weaviateResult =
          await weaviateService.storeCompanyEthics(companyEthics);
    ...
    console.log(
      `🎉 Batch analysis completed in ${duration}s - Success: ${successful.length}, Failed: ${failed.length}`
    );
